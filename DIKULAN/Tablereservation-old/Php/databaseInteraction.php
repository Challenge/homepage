...
<?php
	...
	/* Change the seat value for the given ticket to the given seat*/
	function changeReservation($seat, $ticket){
			$sql = 'UPDATE reservationstable 
					SET seat_number=? WHERE ticket_id = ?';
			$input = array($seat,$ticket);
			return doSQL($sql,$input)->errorCode();
	}
	
	/* Deletes a reservation with the given ticket Id*/
	function cancelReservation($ticket){
		$sql = 'DELETE FROM reservationstable 
				WHERE ticket_id = ?';
			$input = array($ticket);
			return doSQL($sql,$input)->errorCode();
	}
	...
	function makeReservation($seat,$ticket){
			$sql = 'INSERT INTO reservationstable (seat_number, ticket_id ) VALUES (?,?)';
			$input = array($seat,$ticket);
			return doSQL($sql,$input)->errorCode();	
	}
	...
	function getTakenSeats(){
		$takenArray = array();
		$sql = 'SELECT seat_number, ticket_id FROM reservationstable';
	    $arr = doSQL($sql,array())->fetchAll();
	
		foreach($arr as $row){
			$takenArray[$row[0]] = $row[1];
		}
