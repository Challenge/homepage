...
<script type="text/javascript"> 
	...
	*/
	...
	if(isset($_REQUEST['style'])){?>
		document.getElementById('themeMenu').value = "<?php echo $_REQUEST['style']; ?>";
	<?php
	} else {
	?>
		document.getElementById('themeMenu').value = "<?php global $defaultTheme; echo $defaultTheme?>";
	<?php
	}
	...
	/* Selected all the previously selected seats */
		for($i = 0; isset($_REQUEST['selected'.$i]);$i++){
			...
			if( $ticket == 'undefined'){
				global $defaultTextboxText;
				 $ticket = $defaultTextboxText;
			}
			
			echo 'setTextBoxValue('.$seat.',"'.$ticket.'");';
